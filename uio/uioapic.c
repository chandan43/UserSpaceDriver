...
Use case   : modprobe uioapic 
	   : insmod uioapicint.ko irq=1 //keyboard
	   : insmod uioapicint.ko irq=21 //USB keyboard
*/
...
static void uioapic_release(struct device *dev)
{
	pr_info("%s: Releasing my uioapic device\n",__func__);
}
 ...
 */
static int uioapic_mmap(struct uio_info *info, struct vm_area_struct *vma)
{
        unsigned long region_origin = vma->vm_pgoff * PAGE_SIZE;
        unsigned long region_length = vma->vm_end - vma->vm_start;
        unsigned long physical_addr = IOAPIC_BASE + region_origin;
...
static irqreturn_t uioapic_handler(int irq, struct uio_info *dev_info)
{
	static int count = 0;
...
/**
 * uioapic_register_device - register a new userspace IO device
 ...
 */
static int __init uioapic_init(void)
{
	int retval;
	dev = kzalloc(sizeof(struct device), GFP_KERNEL);
	dev_set_name(dev, "uioapic_dev");
	dev->release = uioapic_release;
	...
	/* irq information */
	info->name = "uioapic_device";
	info->version = "0.1";
	info->irq = irq;
	info->irq_flags = IRQF_SHARED;
	info->handler = uioapic_handler;
	info->mmap = uioapic_mmap; 
	info->mem[0].addr = IOAPIC_BASE;
...
static void __exit uioapic_exit(void)
{
...
module_init(uioapic_init);
module_exit(uioapic_exit);
